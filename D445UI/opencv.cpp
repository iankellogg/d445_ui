...
void processKnobMatch(vector<Point> &Knob, Mat &OutputFrame)
{
    ...
    double buffer = -90-(atan2(Knob[minDistancePoint].y-knob_center.y,Knob[minDistancePoint].x-knob_center.x)*180.0/M_PI);
    static double theta;
    theta = .90*theta + 0.1*buffer;
    ...
    //putText(OutputFrame, angleText, Point(50,frame.rows-50), FONT_HERSHEY_SIMPLEX, 0.75, Scalar(0,0,255), 1, LINE_AA);
    fillPoly(OutputFrame,Knob,Scalar(0,0,255));
    line(OutputFrame,knob_center,Knob[minDistancePoint],Scalar(0,255,0));

    draw_tray(OutputFrame, 20, 300, knob_center,theta+270);
